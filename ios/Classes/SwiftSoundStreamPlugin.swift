...
public class SwiftSoundStreamPlugin: NSObject, FlutterPlugin {
  ...
  public static func register(with registrar: FlutterPluginRegistrar) {
    let channel = FlutterMethodChannel(
      name: "vn.casperpas.sound_stream:methods", binaryMessenger: registrar.messenger())
...
//
  ...
  /** ======== Plugin methods ======== **/
    ...
    func routeChangeNotificationListener() {
    ...
    func removeChangeRouteListener(){
        let nc = NotificationCenter.default
...
//    startEngine()
  ...
  private func startEngine() {
    ...
    do {
      ...
      print("mAudioEngine start error")
    }
  }
  ...
  private func initializePlayer(_ call: FlutterMethodCall, _ result: @escaping FlutterResult) {
    guard let argsArr = call.arguments as? [String: AnyObject]
      ...
      sendResult(
        result,
        FlutterError(
          code: SoundStreamErrors.Unknown.rawValue,
    ...
    mPlayerInputFormat = AVAudioFormat(
      commonFormat: AVAudioCommonFormat.pcmFormatInt16, sampleRate: mPlayerSampleRate, channels: 1,
      interleaved: true)
  ...
  private func initializeRecorder(_ call: FlutterMethodCall, _ result: @escaping FlutterResult) {
    ...
    debugLogging = argsArr["showLogs"] as? Bool ?? debugLogging
    mRecordFormat = AVAudioFormat(
      commonFormat: AVAudioCommonFormat.pcmFormatInt16, sampleRate: mRecordSampleRate, channels: 1,
      interleaved: true)
  ...
  private func startRecorder() {
    ...
    guard let converter = AVAudioConverter(from: inputFormat, to: mRecordFormat!) else {
      print("converter errro: \(inputFormat)")
      return
    }
    ...
    input.installTap(onBus: mRecordBus, bufferSize: mRecordBufferSize, format: inputFormat) {
      ...
      let convertedBuffer = AVAudioPCMBuffer(
        pcmFormat: self.mRecordFormat!, frameCapacity: UInt32(Float(buffer.frameCapacity) / ratio))!

      var error: NSError?
      let status = converter.convert(
        to: convertedBuffer, error: &error, withInputFrom: inputCallback)
      assert(status != .error)
  ...
  private func sendMicData(_ data: [UInt8]) {
    let channelData = FlutterStandardTypedData(
...
//      startEngine()
  ...
  private func writeChunk(_ call: FlutterMethodCall, _ result: @escaping FlutterResult) {
    guard let argsArr = call.arguments as? [String: AnyObject],
      ...
      sendResult(
        result,
        FlutterError(
          code: SoundStreamErrors.FailedToWriteBuffer.rawValue,
  ...
  private func bytesToAudioBuffer(_ buf: [UInt8]) -> AVAudioPCMBuffer {
    let frameLength =
      UInt32(buf.count) / mPlayerInputFormat.streamDescription.pointee.mBytesPerFrame
    ...
    buf.withUnsafeBufferPointer {
      let src = UnsafeRawPointer($0.baseAddress!).bindMemory(
        to: Int16.self, capacity: Int(frameLength))
